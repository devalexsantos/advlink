...
import { Image as ImageIcon, Link2, Palette, ShieldCheck, Sparkles, MapPin, Calendar, Rocket, Layers, Eye, LayoutGrid, FileText } from "lucide-react"
    ...
    <section className="relative px-6 py-8">
      <div className="mx-auto max-w-6xl">
        <motion.h2
          ...
          className="text-center text-3xl md:text-5xl font-extrabold tracking-tight mb-10 text-zinc-900"
        >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {items.map((it, idx) => (
            <motion.div
              ...
              className="rounded-2xl border border-zinc-200 bg-white p-5 hover:bg-zinc-50 transition-colors"
            >
              <div className="flex items-start gap-3">
                <div className="mt-0.5 rounded-md border border-zinc-200 bg-white p-2">
                  <it.icon className="w-5 h-5 text-zinc-700" />
                </div>
                <div>
                  <h3 className="font-semibold text-zinc-900">{it.title}</h3>
                  <p className="text-sm text-zinc-600 leading-relaxed mt-1">{it.desc}</p>
                </div>
              </div>
            </motion.div>
          ))}
      ...
      <div className="pointer-events-none absolute inset-0 -z-10 opacity-[0.08]">
        <div className="h-full w-full" style={{ backgroundImage: "linear-gradient(transparent 95%, rgba(0,0,0,.08) 95%), linear-gradient(90deg, transparent 95%, rgba(0,0,0,.08) 95%)", backgroundSize: "30px 30px" }} />
      </div>
    </section>
  )
}
