...
export async function POST(req: Request) {
  ...
  if (!["image-left", "image-right", "text-only"].includes(layout)) {
  ...
  const section = await prisma.customSection.create({
    data: { userId, title, description, imageUrl, layout, iconName, position },
  })
...
export async function PATCH(req: Request) {
  ...
  const section = await prisma.customSection.update({
    where: { id },
    data: { title, description, imageUrl, layout, iconName },
  })
