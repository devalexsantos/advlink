...
export async function POST(req: Request) {
  ...
  const last = await prisma.gallery.findFirst({ where: { profileId }, orderBy: { position: "desc" }, select: { position: true } })
  const nextPosition = (last?.position ?? 0) + 1

  const arrayBuffer = await cover.arrayBuffer()
  const ext = cover.type.split("/")[1] || "jpg"
  const key = `gallery/${profileId}.${Date.now()}.${Math.random().toString(36).slice(2, 8)}.${ext}`
  ...
  const created = await prisma.gallery.create({
    data: { profileId, coverImageUrl: uploaded.url, position: nextPosition },
  })
...
export async function PATCH(req: Request) {
  ...
  const owned = await prisma.gallery.findMany({ where: { id: { in: ids }, profileId }, select: { id: true } })
...
export async function DELETE(req: Request) {
  ...
  const existing = await prisma.gallery.findFirst({ where: { id, profileId } })
  if (!existing) return NextResponse.json({ error: "Not found" }, { status: 404 })

  await prisma.gallery.delete({ where: { id } })
  const remaining = await prisma.gallery.findMany({ where: { profileId }, orderBy: { position: "asc" } })
